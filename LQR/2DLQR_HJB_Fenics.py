...
nx, ny = 149,149
mesh = RectangleMesh(Point(-4.5, -4.5), Point(1.5, 1.5), nx, ny) # 
...
K = Expression((("0.16","0.0"),("0.0","0.16")), degree = 0) # diffusion matrix
...
F = inner(u0-u, v)*dx- 0.5*dt*inner(K*grad(u), grad(v))*dx -0.5 *dt*dot(nabla_grad(u),nabla_grad(u))*v*dx 

# Prepare solution function and solver
# Time-stepping
t = 1.0
u.rename("u", "temperature")
u0.vector()[:] = 50*np.sum((V.tabulate_dof_coordinates()-np.array([[0.0,0.0]]))**2,1)
