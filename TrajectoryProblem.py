...
class TrajectoryProblem(AbstractOCProblem):
    ...
    def __init__(self):
        super().__init__()
        self.d = 2
        self.xtarget = torch.tensor([1.5, 1.5]).reshape(1,2)
    ...
    def x_init(self,nex):
        return -1.5 * torch.ones(nex,2) + 1.0*torch.randn(nex,2)
    ...
    def render(self, s,z,dw,Phi,sPath, nx=100):
        ...
        Q = 50. * normpdf(XY, mu=self.mu, cov=self.cov)
...
class TrajectoryProblem2(TrajectoryProblem):
    ...
    def sigma(self, t, x):
        ...
        '''
        sigma = torch.tensor([0.2,-0.4,-0.4,0.2]).reshape(2,2)
        return sigma
