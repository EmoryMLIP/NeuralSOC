...
parser.add_argument('--prob', choices=['Benchmark','Benchmark2'],   type=str, default='Benchmark')
parser.add_argument('--sampler', choices=['PMPSampler', 'RandomWalk'],   type=str, default='PMPSampler')
...
parser.add_argument("--n_plot"    , type=int, default=2, help="number of plot examples")
parser.add_argument("--nt_val", type=int, default=40, help="number of time steps for validation")
parser.add_argument('--beta'  , type=str, default='1.0, 1.0, 1.0, 0.0, 1.0, 1.0') # BSDE penalty, Terminal, grad terminal, HJB, J, Phi(0)-J; Note: Terminal already has a weight of 100
parser.add_argument('--m'     , type=int, default=64, help="NN width")
...
if __name__ == '__main__':
    ...
    if args.net == "ResNet_OTflow":
        ...
        Phi = Phi_OTflow(4, args.m, prob.d)
    ...
    elif args.sampler == "RandomWalk":
        # only use random walk sampling for comparison purposes, not recommended otherwise as we explained in our work
        from fsde import RandomWalkSampler, PMPSampler
        # need to change the definition for L if using Randomwalk sampler
        def func_tmp(t,x,u):
            return  -torch.norm(u, dim=1, keepdim=True)**2
        prob.L = func_tmp
        sampler = RandomWalkSampler(prob,prob.t,prob.T,args.nt_train)
    ...
    if args.prob == 'Benchmark2':
        sampler2 = PMPSampler(Phi, prob, prob.t,prob.T,args.nt_train)
        save_array = np.zeros(args.n_iters)
    for itr in range(1, args.n_iters + 1):
        ...
        if args.prob == 'Benchmark2':
            x_arr = prob.x_init(args.n_train)
            s, z, dw, Phiz, gradPhiz = sampler2(x_arr)
            _, L, G = control_obj(Phi, prob, s, z, gradPhiz)
            J = torch.abs(L)+G
            save_array[itr-1] = J.detach().numpy()
        ...
        if itr % args.print_freq == 0:
            ...
            print('Training time between each print : %.2f secs' % (elapsed))
            start_time = time.time()

        if itr % args.val_freq == 0 or itr == args.n_iters-1:
            s, z, dw, Phiz, gradPhiz = sampler(x)
            J, L, G = control_obj(Phi, prob, s, z, gradPhiz)
            term_Phi, term_gradPhi = terminal_penalty(Phi, prob, s, z, dw, Phiz, gradPhiz)
            cBSDE = bsde_penalty(Phi, prob, s, z, dw, Phiz, gradPhiz)
        ...
        if itr % args.viz_freq == 0 and args.prob == 'Benchmark':
            s, z, dw, Phiz, gradPhiz = sampler(xp)
    ...
    if args.prob == 'Benchmark2':
        np.save(os.path.join(args.save, 'save_array.npy'), save_array)
